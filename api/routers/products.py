...
async def create_product(
    ...
    available: bool = Form(...),
    price_kg: float = Form(None),
    price_item: float = Form(None),
    ...
    if (price_kg is None and price_item is None) or (price_kg and price_item):
        raise HTTPException(
            ...
            detail="Either price_kg or price_item must be specified and also not both at the same time",
        )
    ...
    query = (
        ...
        .filter(
            ...
            and Product.category_name == product.category_name
        )
        .first()
    )
...
def get_product(id: int, db: Session = Depends(get_db)):
    ...
    if single_product:
        ...
        raise HTTPException(
            ...
            detail=f"Product with id {id} doesnt exist",
        )
...
async def update_product(
    ...
    if (price_kg is None and price_item is None) or (
        price_kg is not None and price_item is not None
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Either price_kg or price_item must be specified and also not both at the same time",
